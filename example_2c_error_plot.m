...
for itr=1:length(rv)
    ...
    abs(cap(itr)-exact_cap)/exact_cap
    rerror(itr) =  abs(cap(itr)-exact_cap)/exact_cap;      
end
%
%%
figure
plot(rv,log10(rerror),'-ob','LineWidth',1.5)
xlabel('{$r$}','FontSize',14,'Interpreter','latex');
ylabel('$\log_{10}$(Relative error)','FontSize',14,'Interpreter','latex');
...
print -depsc fig_BC_E2c_err
% print -dpdf fig_BC_E2c_err
%%
% 
figure
plot(rv,cap,'-ob','LineWidth',1)
xlabel('{$r$}','FontSize',18,'Interpreter','latex');
ylabel('$Cap$','FontSize',18,'Interpreter','latex');
set(gca,'XTick',[0:0.25:1],'FontSize',18);
