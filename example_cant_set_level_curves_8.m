...
m        =  length(Lc);
alphav   =  cent;
deltav   =  zeros(size(alphav)); deltav(m/2+1:m)=1;
mp       =   m; 
ell      =   0;
%
%
%
[x,y] =  meshgrid(linspace(-0.6,1.6,1001),linspace(-0.6,0.6,1001));
z     =  x+i.*y;
for k=1:m
...
figure;
hold on
tv = [0.0488,0.08535,0.1:0.05:0.9,0.91465,0.9512];
...
axis equal
axis([-0.25  1.25 -0.25  0.25])
% axis off
box on
ax=gca;
set(gca,'XTick',[-0.25:0.25:1.25],'FontSize',14);
set(gca,'YTick',[-0.25:0.25:0.25]);
set(gca,'LooseInset',get(gca,'TightInset'))
print -depsc fig_cant_set_lc8
% print -dpdf  fig_cant_set_lc8
%